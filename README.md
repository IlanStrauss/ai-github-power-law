...
Our method combines power law estimation with mechanism diagnostics. We first estimate power law exponents using the Clauset-Shalizi-Newman (2009) methodology to document concentration trends. We then distinguish between hypotheses using four diagnostic tests: attachment kernel estimation (does growth compound?), rate heterogeneity trends (is the population diversifying?), top-performer composition (who enters the top 1%?), and rank persistence (do the same developers stay at the top?).
