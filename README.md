...
**Method**. Our method combines power law estimation with diagnostics to test the underlying mechanism at play. We estimate power law exponents using the Clauset-Shalizi-Newman (2009) methodology to document concentration trends, comparing power law and lognormal fits. We track developers in a panel across years, separating organization developers (contributing to company-owned repositories) from personal developers. We then apply four diagnostic tests to distinguish between hypotheses: attachment kernel estimation (does growth compound?), Poisson vs. Negative Binomial comparison (is the population heterogeneous?), top-performer composition analysis (who enters the top 1%?), and rank persistence measurement (do the same developers stay at the top?).

**Results**. We find that concentration increased — the power law exponent $\alpha$ declined from 2.0 to 1.8 — but this reflects a more heterogeneous user base, not "rich-get-richer" dynamics. The evidence **strongly supports Hypothesis B**. The attachment kernel coefficient **$\beta \approx 0.4$** indicates mean reversion rather than compounding advantage. The dispersion of underlying commit rates widened by 46% ($r$: 0.51 → 0.24, $p = 0.009$), consistent with a diversifying user base. The five-year rank correlation is just **$\rho \approx 0.18$**: the superstars of 2019 are largely *not* the superstars of 2024.
...
**The Exception — and Why It Matters.** Among *organization* developers in 2023–2024, "increased activity" overtook "new accounts" as the dominant source of top 1% entrants, with a median year-on-year growth ratio of **1,135×** — compared with 70–100× in prior years. This is the **only pattern in our data consistent with Hypothesis A**, and its timing coincides with enterprise AI tools clearing procurement and security hurdles at large firms. Crucially, **our data ends in October 2025** — just as agentic AI tools (Claude Code, Cursor, Copilot Enterprise at scale) reached widespread deployment. The org exception may be the leading edge of a transition toward Hypothesis A among professional developers that intensified after our data ends. If so, the pattern we document for 2023–2024 understates the effect that future research will uncover.
...
A caveat: "new accounts" here means first appearance in our filtered sample (≥3 commits/year, ≥2 repositories), not necessarily a newly created GitHub account. A developer who was active but below these thresholds in prior years would register as "new" when they first cross them. This could create some mechanical bias toward Hypothesis B if AI tools helped marginal developers cross the activity threshold. However, the magnitude of the effect — 77-81% consistently across five years — and the fact that these developers immediately appear in the *top 1%* (not merely above the minimum threshold) suggests this is not purely a filter artifact.
...
However, **a closer look at the year-over-year trends reveals a pattern consistent with the org developer exception**. For org developers, rank persistence *increased* from ρ = 0.219 (2019-2020) to ρ = 0.284 (2022-2023) — a 30% rise coinciding with AI tool adoption. Personal developers showed a similar but smaller increase (0.277 → 0.326). This is evidence for Hypothesis A that should not be dismissed: rank persistence among professional developers increased during the AI period, even if the absolute levels remain moderate. The finding aligns with our top-1% composition analysis showing "increased activity" overtaking "new accounts" for org developers in 2023-2024. For org developers specifically, the data show *both* higher persistence *and* incumbent amplification — the signature of emerging preferential attachment.

For personal developers, the overall pattern remains more consistent with Hypothesis B. The five-year rank correlation is just ρ ≈ 0.18, and only 27% of top-10% developers in 2019 remained in the top 10% in 2024. This is close to what we would expect by chance. The superstars of 2019 are largely not the superstars of 2024.
