#!/usr/bin/env python3
"""
Bootstrap Power Law Analysis - ORG DEVELOPERS ONLY

Run this first, then run 13b_bootstrap_personal.py
"""
...
def main():
    print("=" * 70)
    print("BOOTSTRAP: ORG DEVELOPERS")
    print(f"N={N_BOOTSTRAP} bootstrap iterations per year")
    print("=" * 70, flush=True)

    # Load the org developer analysis results to get the data
    org_analysis = pd.read_csv(OUTPUT_DIR / "org_developer_analysis.csv")
    org_data = org_analysis[org_analysis["group"] == "org_developers"]

    # We need the raw commit data - load from intermediate files
    from pathlib import Path
    import gzip
    import json
    ...
    # Extract org developer commits by year
    print("\nExtracting org developer commits...", flush=True)
    raw_files = sorted(RAW_DIR.glob("*.json.gz"))

    # Collect per-developer stats
    developer_stats = {}
    ...
    # Convert to dataframe and filter for ORG DEVELOPERS
    print("\nFiltering for org developers...", flush=True)
    records = []
    for (year, actor), stats in developer_stats.items():
        if stats["org_commits"] > 0:  # Has at least one org commit
    ...
    print(f"Org developers after filters: {len(df):,}", flush=True)
    ...
    output_file = OUTPUT_DIR / "bootstrap_org_developers.csv"
